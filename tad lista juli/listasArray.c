...
void l_borrar(Lista lista, int clave){
    if (!l_es_vacia(lista)) {
        ...
        while (i < lista->cantidad) {
            ...
            i++;
            if (x > 0 && lista->cantidad != i) {
                lista->valores[i - x]->clave = lista->valores[i]->clave;
            }
        }
        for (i = 0;i < x;i++) {
            lista->valores[lista->cantidad - i] = NULL;
        }
        lista->cantidad -= x;
    }
}
...
//Objetivo: Retorna "true" mientras existen mas elementos por recorrer la lista. Caso contrario
//retorna "False"
bool hay_siguiente(Iterador iterador) {
    return iterador->lista->cantidad - 1 > iterador->posicionActual;
}

//Objetivo: Retorna el proximo elemento de la lista a recorrer
